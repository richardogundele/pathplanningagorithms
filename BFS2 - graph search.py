...
class BreadthFirstSearch:
    ...
    def get_possible_moves(self):
        self.current = self.open_set.pop(list(self.open_set.keys())[0]) #FIFO
    ...
    def explore_next_move(self):
        ...
        for move in potential_moves:
            ...
            if (n_id not in self.closed_set) and (n_id not in self.open_set):
                node[4] = self.current
    ...
    def cal_path_distance(self, current):
        ...
        node = self.closed_set[current[3]]
        while len(node) != 0:
            dx.append(node[0])
            dy.append(node[1])
            node = node[4]
...
bfs = BreadthFirstSearch(start, goal, grid, grid_size, robot_radius, fig_dim, motion_option)
while True:
