...
show_animation = True
road = []
counter = 999 #Just inserted this when correlating
...
class DepthFirstSearch:
    ...
    def get_possible_moves(self):
        self.current = self.open_set.pop(list(self.open_set.keys())[-1]) #LIFO
        self.c_id = self.calc_grid_index(self.current)
    ...
    def cal_path_distance(self, current):
        ...
        index= current[3]                   #node = self.closed_set[current[3]]
        c = 0                                   # while len(node) != 0:
        while index != -1:                      #     dx.append(node[0])
            node = self.closed_set[index]       #     dy.append(node[1])
            dx.append(node[0])                  #     node = node[4]
            dy.append(node[0])
            index = node[3]
            c = c + 1
            if c == counter:
...
if(motion_option != '4n') and (motion_option != '8n') and (motion_option != '6n'): #added 6n myself to try things
...
bfs = DepthFirstSearch(start, goal, grid, grid_size, robot_radius, fig_dim, motion_option)
while True:
